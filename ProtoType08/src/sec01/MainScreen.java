...
public class MainScreen extends JFrame {
	...
	public MainScreen(LoginInfo agent) {
		...
		JLabel loginLabel = agent.loginLabel(465-(agent.getUser().getUname().length()-4)*9, 17, 280+(agent.getUser().getUname().length()-4)*10, 60, new Font("HY엽서L", Font.BOLD, 21));
		loginLabel.setForeground(Color.YELLOW);
		background.add(loginLabel);
		JButton logout = agent.logout(curPage, 515, 70, 85, 35);
		logout.setFont(new Font("HY엽서L", Font.BOLD, 17));
