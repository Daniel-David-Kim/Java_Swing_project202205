	...
	public void loginSettings() {
		...
		ActionListener btnLis = (e) -> {
			...
			if(target.getText().equals("로그인")) {
				...
				HashMap<String, Object> call = dao.select(String.format("select * from members where uid='%s' and upw='%s'", id, pass), "members");
				...
				if(resRow == 0) JOptionPane.showMessageDialog(null, "회원정보가 일치하지 않습니다.", "Error!", JOptionPane.ERROR_MESSAGE);
					...
					if(member.getUclass() == 1) JOptionPane.showMessageDialog(null, String.format("%s님 환영합니다!", member.getUid()), "Complete", JOptionPane.INFORMATION_MESSAGE);
